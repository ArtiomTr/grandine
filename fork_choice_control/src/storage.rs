...
impl<P: Preset> Storage<P> {
    ...
    pub(crate) fn append_finalized_state(
        ...
        if let Some((parent_key, parent_state)) = parent_state {
            ...
            // Nothing can be delta-encoded without an ancestor. A non-leaf falls back to a
            // snapshot; a leaf is skipped, because a leaf snapshot would be re-encoded as a delta
            // the moment an ancestor shows up. Either way `to_replace` is left alone: deleting the
            // copy already on disk without writing a replacement would lose the state outright.
            if to_replace.is_none() && !self.hierarchy.is_leaf::<P>(&self.config, anchor_slot, slot)
            {
                debug_with_peers!(
                    "saving non-leaf state in slot {slot} as a snapshot because \
