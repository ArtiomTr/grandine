...
pub struct Storage<P: Preset> {
    ...
    checkpoint_head_slot: Arc<Mutex<Option<Slot>>>,
...
impl<P: Preset> Storage<P> {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            frame_cache,
            forward_spine,
            checkpoint_head_slot: Arc::new(Mutex::new(None)),
            metrics,
        })
    }
    ...
    pub(crate) fn append<'cl>(
        ...
        if let Some(head_slot) = self.checkpoint_head_slot()? {
        ...
        if let Some(checkpoint_batch) = checkpoint_batch {
            self.write_state_checkpoint(store_head_slot, checkpoint_batch)?;
        }

        Ok(slots)
    }

    fn write_state_checkpoint(&self, head_slot: Slot, batch: Vec<(String, Vec<u8>)>) -> Result<()> {
        let mut cached = self.checkpoint_head_slot.lock();

        if let Some(stored_head_slot) = self.cached_head_slot(&mut cached)?
            && head_slot < stored_head_slot
        {
            debug_with_peers!(
                "skipping checkpoint at head slot {head_slot}: \
                 the stored one is at head slot {stored_head_slot}",
            );
    ...
    fn state_with_key_by_block_root(
        ...
        for (delta, layer, block_root) in deltas {
            ...
            let appended = validators_before..frame.validators().len_usize();
    ...
    pub(crate) fn checkpoint_head_slot(&self) -> Result<Option<Slot>> {
        let mut cached = self.checkpoint_head_slot.lock();

        self.cached_head_slot(&mut cached)
    }

    fn cached_head_slot(&self, cached: &mut Option<Slot>) -> Result<Option<Slot>> {
        if cached.is_none() {
            *cached = self.load_checkpoint_head_slot()?;
        }

        Ok(*cached)
    }

    fn load_checkpoint_head_slot(&self) -> Result<Option<Slot>> {
        let Some(value_bytes) = self.database.get(StateCheckpoint::<P>::KEY)? else {
            return Ok(None);
        };

        // A value too short to hold the slot leaves an empty slice, which `Slot` rejects.
        let slot_bytes = value_bytes
            .get(StateCheckpoint::<P>::HEAD_SLOT_RANGE)
            .unwrap_or_default();

        Slot::from_ssz_default(slot_bytes)
            .map(Some)
            .map_err(Into::into)
    }
    ...
    fn load_state_checkpoint(
        ...
        self.restore_validators_to_state(checkpoint.state.make_mut(), finalized_validators, None)?;
    ...
    fn restore_validators_to_state(
        ...
        match finalized_validators {
            Some(validators) => {
                Self::restore_pubkeys_to_state(state, validators.pubkeys(), range)
            ...
            None => {
                ...
                Self::restore_pubkeys_to_state(state, &pubkeys, range)
    ...
    fn restore_pubkeys_to_state(
        ...
        match range {
            Some(range) => validators.restore_pubkeys_in(pubkeys, range),
            None => validators.restore_pubkeys(pubkeys),
        }
    }
...
impl<P: Preset> StateCheckpoint<P> {
    ...
    /// Byte range `head_slot` occupies in a serialized checkpoint.
    const HEAD_SLOT_RANGE: Range<usize> = {
        let start = <H256 as SszSize>::SIZE.fixed_part();

        start..start.saturating_add(<Slot as SszSize>::SIZE.fixed_part())
    };
}
...
mod tests {
    ...
    fn checkpoint_batch(
        ...
        Ok(vec![serialize(
            ...
            StateCheckpoint {
                block_root,
                head_slot,
                state: state_with_slot(state_slot),
            },
        )?])
    }

    #[test]
    fn the_head_slot_is_read_without_decoding_the_checkpoint_state() -> Result<()> {
        let storage = Storage::<Mainnet>::new(
            Arc::new(Config::mainnet()),
            Arc::new(PubkeyCache::default()),
            Database::in_memory(),
        ...
        assert_eq!(storage.load_checkpoint_head_slot()?, None);

        storage
            .database
            .put_batch_raw(checkpoint_batch(4096, H256::repeat_byte(1), 4064)?)?;

        assert_eq!(storage.load_checkpoint_head_slot()?, Some(4096));

        // The slot must be the one the container carries, not the state's own.
        let StateCheckpoint {
            head_slot, state, ..
        } = storage
            .get::<StateCheckpoint<Mainnet>>(StateCheckpoint::<Mainnet>::KEY)?
            .expect("the checkpoint was just written");

        assert_eq!(head_slot, 4096);
        assert_eq!(state.slot(), 4064);

        Ok(())
    }

    #[test]
    fn a_checkpoint_write_never_rewinds_the_stored_head_slot() -> Result<()> {
        let storage = Storage::<Mainnet>::new(
            Arc::new(Config::mainnet()),
            Arc::new(PubkeyCache::default()),
            Database::in_memory(),
        ...
        let later = H256::repeat_byte(1);
        let earlier = H256::repeat_byte(2);
        let same = H256::repeat_byte(3);

        storage.write_state_checkpoint(4096, checkpoint_batch(4096, later, 4064)?)?;

        assert_eq!(storage.checkpoint_head_slot()?, Some(4096));

        // An older run that commits late must not take the anchor back with it.
        storage.write_state_checkpoint(2048, checkpoint_batch(2048, earlier, 2016)?)?;
        ...
        // A run that archives a later anchor while the head stands still still gets through.
        storage.write_state_checkpoint(4096, checkpoint_batch(4096, same, 4096)?)?;
    ...
    #[test]
    fn a_checkpoint_write_respects_a_head_slot_left_by_an_earlier_run() -> Result<()> {
        let storage = Storage::<Mainnet>::new(
            Arc::new(Config::mainnet()),
            Arc::new(PubkeyCache::default()),
            Database::in_memory(),
        ...
        // Written behind the guard's back, as a previous run of the node would have left it.
        storage
            .database
            .put_batch_raw(checkpoint_batch(4096, stored, 4064)?)?;

        storage
            .write_state_checkpoint(2048, checkpoint_batch(2048, H256::repeat_byte(2), 2016)?)?;

        assert_eq!(
            storage
                .get::<StateCheckpoint<Mainnet>>(StateCheckpoint::<Mainnet>::KEY)?
                .expect("the stored checkpoint must survive")
                .block_root,
            stored,
        );
