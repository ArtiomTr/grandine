...
use execution_engine::{
    ...
    PayloadAttributesV2, PayloadAttributesV3, PayloadId, PayloadStatusV1,
};
...
use crate::{auth::Auth, deposit_event::DepositEvent, eth1_block::Eth1Block, Eth1ApiToMetrics};
...
impl Eth1Api {
    ...
    fn get_adapter(&self) -> Result<&Box<dyn EmbedAdapter>> {
        CURRENT_ADAPTER
            .get()
            .ok_or(Error::AdapterNotInitialized.into())
    }

    pub async fn current_head_number(&self) -> Result<ExecutionBlockNumber> {
        self.get_adapter()?.eth_block_number()
    }

    pub async fn get_block(&self, block_id: BlockId) -> Result<Option<Eth1Block>> {
        match block_id {
            BlockId::Hash(hash) => self.get_adapter()?.eth_get_block_by_hash(hash),
            BlockId::Number(number) => self.get_adapter()?.eth_get_block_by_number(number),
        }
    }
    ...
    pub async fn get_first_deposit_contract_block_number(
        ...
        let logs = self.get_adapter()?.eth_logs(filter)?;
    ...
    pub async fn get_deposit_events(
        ...
        for log in self.get_adapter()?.eth_logs(filter.clone())? {
