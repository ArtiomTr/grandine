...
Writing finalized states to disk happens off the fork choice thread, on a pool of archiver threads that grows to at most half the machine's cores and shrinks back to none once its queue drains. Every queued or running archival task owns a snapshot of the store, so peak memory during a long archival pass scales with how much archival work is outstanding.
