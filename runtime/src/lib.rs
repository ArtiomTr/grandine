#![allow(
    unused_crate_dependencies,
    reason = "tokio_util is used only for embed feature, thus emits warning that it is unused, when compiling without embed"
)]
pub use crate::{
    defaults::{
        DEFAULT_DATABASE_ZSTD_COMPRESSION_LEVEL, DEFAULT_ETH1_DB_SIZE, DEFAULT_ETH2_DB_SIZE,
        DEFAULT_LIBP2P_IPV4_PORT,
        DEFAULT_LIBP2P_IPV6_PORT, DEFAULT_LIBP2P_QUIC_IPV4_PORT, DEFAULT_LIBP2P_QUIC_IPV6_PORT,
        DEFAULT_METRICS_PORT, DEFAULT_METRICS_UPDATE_INTERVAL_SECONDS,
        DEFAULT_RECONSTRUCTION_DELAY_MS, DEFAULT_REQUEST_TIMEOUT, DEFAULT_TARGET_PEERS,
        DEFAULT_TARGET_SUBNET_PEERS, DEFAULT_TIMEOUT, default_network_config,
    },
    misc::{MetricsConfig, StorageConfig},
    runtime::{RuntimeConfig, run, run_after_genesis},
    schema::initialize as initialize_schema,
};

#[cfg(feature = "embed")]
pub use crate::runtime::shutdown;

pub mod commands;
mod config_dir;
mod consts;
mod db_info;
pub mod db_stats;
mod defaults;
pub mod grandine_args;
pub mod grandine_config;
mod misc;
pub mod predefined_network;
mod runtime;
mod schema;
mod validators;
