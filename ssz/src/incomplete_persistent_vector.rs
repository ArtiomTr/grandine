    ...
    TailElements<N, B>: ArrayLength<T>,
{
    ...
    fn try_from_iter(elements: impl IntoIterator<Item = T>) -> Result<Self, Self::Error> {
        ...
        for _ in 0..Self::depth() {
            ...
            if let Some(left_over) = left_over.take() {
                run_length_encoded_nodes.push((left_over, 1));
            }
        }
    ...
    TailElements<N, B>: ArrayLength<T>,
{
        ...
        N: Unsigned + NonZero,
    {
        ...
        let mut bundle_index = B::index_of_bundle(index);
        let mut leaf_count = Self::actual_leaf_count();
        let mut node = self.root.as_ref().as_ref();
        let bundle = loop {
            match node {
                Node::Internal { left, right, .. } => {
                    let left_count = leaf_count.next_power_of_two() / 2;

                    if bundle_index < left_count {
                        node = left;
                    } else {
                        bundle_index -= left_count;
