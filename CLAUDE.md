...
They sit behind the `comparison` feature, so that bench needs it:

```
REMOTE_URL='...' cargo bench -p diff --features comparison --bench comparison
```

`xdelta3` is built from source, so a C toolchain is needed for that feature.
