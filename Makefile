...
release:
ifeq ($(TARGET),)
	cargo build --profile compact --bin grandine --features $(FEATURES) $(EXCLUDES)
else
	@make $(TARGET)
endif
...
./target/x86_64-unknown-linux-gnu/compact/grandine:
	cross build --bin grandine --target x86_64-unknown-linux-gnu --features $(FEATURES) --profile compact $(EXCLUDES)
...
./target/aarch64-unknown-linux-gnu/compact/grandine:
	cross build --bin grandine --target aarch64-unknown-linux-gnu --features $(FEATURES) --profile compact $(EXCLUDES)
...
./target/x86_64-pc-windows-msvc/compact/grandine.exe:
	cargo build --profile compact --bin grandine --target x86_64-pc-windows-msvc --features $(FEATURES) $(EXCLUDES)
...
./target/x86_64-apple-darwin/compact/grandine:
	cargo build --profile compact --bin grandine --target x86_64-apple-darwin --features $(FEATURES) $(EXCLUDES)
...
./target/aarch64-apple-darwin/compact/grandine:
	cargo build --profile compact --bin grandine --target aarch64-apple-darwin --features $(FEATURES) $(EXCLUDES)
