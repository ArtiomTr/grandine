...
mod spec_tests {
    ...
    #[test_resources("consensus-spec-tests/tests/mainnet/gloas/rewards/*/*/*")]
    fn mainnet(case: Case) {
        run_case::<Mainnet>(case).expect("gloas rewards test should not return an error")
    }

    #[test_resources("consensus-spec-tests/tests/minimal/gloas/rewards/*/*/*")]
    fn minimal(case: Case) {
        run_case::<Minimal>(case).expect("gloas rewards test should not return an error")
    }
