...
mod spec_tests {
    ...
    fn mainnet(case: Case) {
        run_case::<Mainnet>(case).expect("electra rewards test should not return an error")
    }

    #[test_resources("consensus-spec-tests/tests/minimal/electra/rewards/*/*/*")]
    fn minimal(case: Case) {
        run_case::<Minimal>(case).expect("electra rewards test should not return an error")
    }
