...
use anyhow::Result;
use arithmetic::{ArithmeticError, NonZeroExt as _, U64Ext as _, UsizeExt as _};
...
pub fn process_slashings<P: Preset, S: SlashingPenalties>(
    ...
    let mut apply_slashing = |balance: &mut Gwei| -> Result<()> {
        ...
        let penalty_per_effective_balance_increment =
            adjusted.try_div(total_active_balance / increment)?;

        // > [Modified in Electra:EIP7251]
        let penalty =
            penalty_per_effective_balance_increment.try_mul(effective_balance_increments)?;
...
mod spec_tests {
    ...
    #[test_resources(
        "consensus-spec-tests/tests/mainnet/electra/epoch_processing/justification_and_finalization/*/*"
    )]
    ...
    #[test_resources(
        "consensus-spec-tests/tests/minimal/electra/epoch_processing/justification_and_finalization/*/*"
    )]
    ...
    #[test_resources("consensus-spec-tests/tests/mainnet/electra/epoch_processing/*/*/*")]
    fn mainnet_epoch_processing(case: Case) {
        run_epoch_case::<Mainnet>(case);
    }

    #[test_resources("consensus-spec-tests/tests/minimal/electra/epoch_processing/*/*/*")]
    ...
    fn run_effective_balance_updates_case<P: Preset>(case: Case) {
        run_case::<P>(case, |_, state| process_effective_balance_updates(state));
    }
    ...
    fn run_epoch_case<P: Preset>(case: Case) {
        ...
        let Some(mut state) = case.try_ssz_default::<ElectraBeaconState<P>>("pre_epoch") else {
            return;
        };
        let post_option: Option<ElectraBeaconState<P>> = case.try_ssz_default("post_epoch");
