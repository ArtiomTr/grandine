...
fn run_rounds<P: Preset, T: Send>(slice: &mut [T], seed: H256, length: NonZeroU64, parallel: bool) {
...
fn swap_around_mirror<T: Send>(
    ...
    let source_base = (offset_length / BITS_PER_HASH).saturating_sub(1);
    let low_main = &mut low[trailing..mirror.saturating_sub(leading)];
    let high_main = &mut high[leading..mirror.saturating_sub(trailing)];

    debug_assert_eq!(low_main.len(), chunk_count.saturating_mul(BITS_PER_HASH));
    debug_assert_eq!(high_main.len(), chunk_count.saturating_mul(BITS_PER_HASH));
...
fn swap_chunk_range<T>(seed: H256, round: u8, low: &mut [T], high: &mut [T], source_base: usize) {
    for (index, (low_chunk, high_chunk)) in low
        ...
        .enumerate()
    {
        let source = compute_source(seed, round, source_base.saturating_sub(index));
...
fn swap_main_chunks<T: Send>(
    ...
    rayon::scope(|scope| {
        for (worker, (low_part, high_part)) in pieces {
            let base = source_base.saturating_sub(worker.saturating_mul(part_chunks));
